...
class DWAConfig:
    ...
    w_heading: float = 1.0
    w_speed: float = 3.0
    w_motion: float = 3.0             # motion-direction bias (pass behind)
    w_smooth: float = 2.0             # temporal action smoothing
    w_commit: float = 4.0             # side-commitment switching penalty
...
class DWAPlanner:
    ...
    def __init__(self, env_cfg: dict, dwa_cfg: DWAConfig | None = None):
        ...
        self._prev_act = np.array([1.5, 0.0], dtype=np.float32)
        self._committed_side = 0.0   # +1 = pass left, -1 = pass right, 0 = uncommitted
    ...
    def _cost_motion_dir(self, lat: float, h_v_lat: float,
                         h_dist: float) -> float:
        """Penalise candidates that cut *in front of* a laterally-moving human.

        ``h_v_lat`` is the human's lateral velocity in the **robot frame**
        (positive = moving to the robot's left).  To pass *behind* the human
        the robot should go to the opposite side, i.e. ``sign(lat)`` should
        differ from ``sign(h_v_lat)``.

        The penalty is proportional to both the human's lateral speed and
        the candidate's lateral offset in the "wrong" direction, and fades
        out when the human is far away.
        """
        if abs(h_v_lat) < 0.05:
            return 0.0
        cross_front = max(0.0, lat * h_v_lat)
        proximity = max(0.0, 1.0 - h_dist / (self.safety_dist * 4.0))
        return cross_front * proximity

    def _cost_smooth(self, fwd: float, lat: float) -> float:
        """Penalise large jumps from the previous action to discourage jitter."""
        d_fwd = fwd - self._prev_act[0]
        d_lat = lat - self._prev_act[1]
        return d_fwd * d_fwd + d_lat * d_lat

    def _cost_commit(self, lat: float, h_rf_y: float) -> float:
        """Penalise switching avoidance side once committed.

        ``h_rf_y`` is the human's lateral position in the robot frame.
        The *pass side* is defined as sign(lat - h_rf_y):  positive means
        the candidate passes to the human's left, negative to the right.
        If committed_side != 0 and the candidate disagrees, a fixed penalty
        is returned.
        """
        if self._committed_side == 0.0:
            return 0.0
        pass_side = np.sign(lat - h_rf_y) if abs(lat - h_rf_y) > 0.15 else 0.0
        if pass_side != 0.0 and pass_side != self._committed_side:
            return 1.0
        return 0.0
    ...
    def plan(self, env: LocalPlannerEnv):
        ...
        if env._human_visible:
            ...
            dx, dy = env.hx - rx, env.hy - ry
            h_rf_x = cr * dx + sr * dy
            h_rf_y = -sr * dx + cr * dy
            h_v_lat = -sr * env.hvx + cr * env.hvy
            h_dist = float(np.hypot(dx, dy))
            if h_rf_x > 0:
        ...
        for fwd in self.fwd_samples:
            for lat in self.lat_samples:
                ...
                cm = self._cost_motion_dir(lat, h_v_lat, h_dist)
                cs = self._cost_smooth(fwd, lat)
                cc = self._cost_commit(lat, h_rf_y) if h_traj is not None else 0.0
        ...
        self._prev_act = best_act.copy()

        if h_traj is not None and best_traj is not None:
            pass_side = np.sign(best_act[1] - h_rf_y)
            if abs(best_act[1] - h_rf_y) > 0.15:
                self._committed_side = float(pass_side)
        else:
            self._committed_side = 0.0
...
def main():
    ...
    pa.add_argument("--w-motion", type=float, default=3.0)
    pa.add_argument("--w-smooth", type=float, default=2.0)
    pa.add_argument("--w-commit", type=float, default=4.0)
    ...
    dwa_cfg = DWAConfig(
        ...
        w_motion=args.w_motion, w_smooth=args.w_smooth,
        w_commit=args.w_commit, goal_lookahead=args.goal_lookahead,
    )
