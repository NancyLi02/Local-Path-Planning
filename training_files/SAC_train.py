...
2. Train with custom timesteps:
    python SAC_train.py --name sac_v1 --timesteps 500000
...
3. Evaluate final saved model:
    python SAC_train.py --name sac_v1 --eval

4. Evaluate best checkpoint:
    python SAC_train.py --name sac_v1 --eval --use-best

5. Evaluate with on-screen rendering:
...
7. Evaluate with hybrid state machine:
    python SAC_train.py --name sac_v1 --hybrid

8. Evaluate hybrid and save videos:
    python SAC_train.py --name sac_v1 --hybrid --save-video

Hybrid videos saved to:
    Evaluation_video/Hybrid_SAC/sac_v1/
...
10. Launch TensorBoard:
    tensorboard --logdir logs/SAC/sac_v1/tb
...
HYBRID_VIDEO_ROOT = _REPO_ROOT / "Evaluation_video" / "Hybrid_SAC"
...
def train(args):
    ...
    n_eval_envs = 4
    train_env = make_env(0, TRAIN_ENV_CFG)()
    eval_env = VecMonitor(
        SubprocVecEnv([make_env(1000 + i, EVAL_ENV_CFG) for i in range(n_eval_envs)])
    )
    ...
    print(f"Training SAC ({name!r}) for {args.timesteps} timesteps ...")
...
def evaluate_hybrid(args):
    ...
    model = SAC.load(str(load_stem))
    policy = HybridPolicy(model, env)

    n_ep = args.eval_episodes
    results = {"SUCCESS": 0, "COLLISION": 0, "TIMEOUT": 0, "OTHER": 0}
    state_stats = {"follow_path": 0, "rl_avoid": 0}

    vid_dir = HYBRID_VIDEO_ROOT / name
    if save_video:
        vid_dir.mkdir(parents=True, exist_ok=True)
        print(f"Hybrid video dir: {vid_dir.resolve()}")

    print(f"Evaluating hybrid with model: {Path(str(load_stem) + '.zip').resolve()}")
    ...
    total_steps = state_stats["follow_path"] + state_stats["rl_avoid"]

    print("\nSummary:")
    for k, v in results.items():
        print(f"  {k}: {v}/{n_ep} ({100 * v / n_ep:.0f}%)")

    print(f"\nState machine usage ({total_steps} total steps):")
    print(
        f"  follow_path: {state_stats['follow_path']} "
        f"({100 * state_stats['follow_path'] / max(total_steps, 1):.1f}%)"
    )
    print(
        f"  rl_avoid:    {state_stats['rl_avoid']} "
        f"({100 * state_stats['rl_avoid'] / max(total_steps, 1):.1f}%)"
    )
...
def reward_audit(args):
    ...
    print(f"Reward version: {REWARD_CFG['reward_version']}")
    print("Per-step mean contribution (approx. scale SAC will see):")
...
if __name__ == "__main__":
    pa = argparse.ArgumentParser(description="Train or evaluate SAC (Stable-Baselines3).")
    pa.add_argument(
        "--name",
        type=str,
        default="sac_run",
    ...
    pa.add_argument(
        "--hybrid",
        action="store_true",
        help="Evaluate with hybrid state machine (pure-pursuit + RL)",
    )
    pa.add_argument(
        "--use-best",
        action="store_true",
        help="When used with --eval or --hybrid, load best/best_model.zip instead of the final <name>.zip",
    )
    pa.add_argument(
        "--reward-audit",
        action="store_true",
