...
class LocalPlannerEnv(gym.Env):
    ...
    DEFAULT_CFG: dict = dict(
        ...
        w_time=-0.5,
        w_success=100,
        # phase-aware path penalty: g(dh) multiplier for deviation/heading
        # g = path_pen_min when dh=0, ramps to 1.0 at dh >= path_pen_restore_dist
        path_pen_min=0.15,
    ...
    def _make_path(self, rng: np.random.Generator) -> None:
        """Generate a straight horizontal reference path with start and goal."""
    ...
    def _reward_terms(self, old_s: float, collision: bool, success: bool) -> dict[str, float]:
        ...
        # Phase-aware path penalty: soften when human is close, full when clear
        ...
        # Return-to-path bonus: when human is clear, reward lateral convergence
        w_return = float(c.get("w_return_path", 3.0))
        if g >= 0.9 and abs(lat) > c["success_lat_thresh"]:
            ret_bonus = w_return * max(0.0, self._prev_abs_lat - abs(lat))
            terms["return_path"] = float(ret_bonus)
            r += ret_bonus
        else:
            terms["return_path"] = 0.0
        self._prev_abs_lat = abs(lat)
...
class HybridPolicy:
    """State machine that switches between pure-pursuit path-following and RL avoidance.

    States
    ------
    FOLLOW_PATH : Robot tracks the reference path via a lookahead-based action.
    RL_AVOID    : RL policy takes over for obstacle avoidance.

    Transitions
    -----------
    FOLLOW_PATH -> RL_AVOID : corridor risk flag fires (human enters the corridor).
    RL_AVOID -> FOLLOW_PATH : human is clear **and** robot is back on the reference path
                             (lateral offset & heading error below thresholds).
    """
    ...
    def __init__(
        ...
        *,
        ...
        if self._normalize:
            scales = _obs_normalization_scales(self.cfg)
    ...
    def _decode_obs(self, obs) -> tuple[float, float, float]:
        """Return (abs_lateral, abs_heading_err, corridor_risk) from raw obs."""
        lat_raw = float(obs[2])
        hdg_raw = float(obs[3])
        risk = float(obs[-1])
    ...
    def predict(self, obs, deterministic: bool = True):
        ...
        if self.state == self.RL_AVOID:
            ...
            action = _obs_to_path_goal(obs, self.cfg, lookahead_idx=self.follow_lookahead)
