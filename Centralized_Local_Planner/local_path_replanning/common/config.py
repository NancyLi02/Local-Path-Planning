"""Local path replanning configuration.
...
class PlannerConfig:
    ...
    # 36 -> 14 over 10k steps; see archive/logs/local_path_replanning/ppo_dropout_run.log).
    dropout: float = 0.0
    # Exploration noise of the Gaussian over raw actions. Kept small: the
    # policy starts from a behaviour clone of the teacher, and loud exploration walks it
    ...
    # outputs/5_local_path_replanning/results/ablation.json.
    ...
    # is safe (the learned planner) does not need to roll out and check the whole backup set
    ...
    # Deployment mode for the learned planner. False (the factory setting, and the only one
    # reported): the learned action is one more candidate and the shield still
    # commits to the lowest-cost safe one, so the learned planner can never be less safe than
    ...
    # and is not used.
    prefer_learned_proposal: bool = False
    ...
    # ---- priority weights: alpha/TTC + beta*task + gamma*braking --------
