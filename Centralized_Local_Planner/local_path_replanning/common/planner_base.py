"""Shared runtime for every local replanning method.

Both planners follow the same contract:

    1. receive a conflict cluster (variable N)
    2. propose one action per AMR       <-- the ONLY difference between the methods
...
class ShieldedReplanner:
    ...
    name = "base"
    # The cost-driven methods rank every safe candidate; the learned one executes its own proposal when
