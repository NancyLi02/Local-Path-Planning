"""Closed-loop runtime: Steps A-D + local path replanning + command execution.
...
Cluster members are handed to the local replanner and leave the QR-cell protocol until they
...
from ...tools.conflict_cluster import ConflictClusterBuilder
from ...tools.factory_map import GOALS, MAP_BOUNDS, OBSTACLES
from ...tools.geometry import point_in_polygon, safety_tube_polygon
from ...tools.prediction import IntentParticlePredictor, PredictorConfig
from ...tools.safety_inflation import (
    SafetyInflationConfig, SafetyInflationModel,
)
from ...tools.scenario import make_amrs, make_workers
...
@dataclass
class ControlState:
    """Local-planner kinematic state of one AMR while the local planner owns it."""
    s: float
    lat: float
    speed: float
    heading: float
...
class LocalReplanningRuntime:
    """A->D pipeline with a pluggable local replanner and command execution."""
    ...
    def step(self, frame: int) -> dict:
        ...
        # ---- local replanning: plan + execute, one cluster at a time ----
    ...
    def current_cluster_agents(self) -> list:
        """ClusterAgent view of every AMR currently under local control."""
