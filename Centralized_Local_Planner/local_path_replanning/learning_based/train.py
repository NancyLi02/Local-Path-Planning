"""PPO trainer for the attention policy (shared actor + centralized critic).

    python -m Centralized_Local_Planner.local_path_replanning.learning_based.train --timesteps 120000

Training follows the specification's plan:

    Stage 1  N = 1        behaviour-clone the teacher's proposal (a competent start)
...
the teacher, so training can never trade safety for reward -- only efficiency.
"""
...
from ..common.action_decoder import decode_action, encode_action_np
...
_LOG = _REPO / "logs" / "local_path_replanning" / "learning_based"
...
def v0_raw_actions(env, cfg) -> np.ndarray:
    """The teacher's EXECUTED action for the env's current cluster, in RAW space.

    The teacher is its committed command, not its raw proposal: most of the
    competence comes from the shield's cost ranking over the candidate set, and
    learned planner executes its own proposal whenever it is safe. Cloning the
    proposal alone would hand it a strictly weaker controller than the teacher.
    """
    teacher = _TEACHER.get(id(cfg))
    if teacher is None:
        teacher = OptimizationBasedReplanner(cfg)
...
def behaviour_clone(policy, env, indices, device, cfg, epochs: int = 300,
    ...
    """Clone the teacher's executed command.

    The loss is taken in DECODED action space, not in the raw pre-sigmoid
    space: the teacher commits saturated actions (goal_fwd = 3.0 m, speed_scale = 1.0),
...
def main(argv=None):
    ...
    pa.add_argument("--bc-coef", type=float, default=0.5,
                    help="behaviour-anchor weight: keeps PPO near the cloned "
                         ...
                         "loses to the teacher (see archive/logs/local_path_replanning/vanilla_ppo_run.log)")
    ...
    pa.add_argument("--name", type=str, default="learning_based")
    pa.add_argument("--device", type=str,
                    default="cuda" if torch.cuda.is_available() else "cpu")
    args = pa.parse_args(argv)

    torch.manual_seed(0); np.random.seed(0)
    cfg = PlannerConfig(); cfg.validate()
    device = args.device
    ...
    base = evaluate(env, policy, eval_idx, device, mode="teacher", cfg=cfg)
    print(f"teacher: ret={base['ret']:7.1f} resolved={base['resolved']*100:5.1f}% "
    ...
    for stage_i, max_n in enumerate(stages[1:], start=2):
        ...
        while gstep < target:
            ...
            print(f"  step {gstep:>7d} ret={ev['ret']:7.1f} resolved={ev['resolved']*100:5.1f}% "
                  ...
                  f"stop={ev['stop']*100:4.0f}%   (teacher ret={base['ret']:.1f})")
