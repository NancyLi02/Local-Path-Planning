"""Runs the speed-adjusting planner inside the shared comparison harness.

This module re-implements nothing. It drives the planner next door
(``speed_adjusting.planner``) through ``Centralized_Local_Planner.main.Pipeline``
-- which is the pipeline that planner was written for -- and only

  * reports the metric vocabulary the other three methods report, and
  * emits the frame snapshots ``..render.build`` draws,

so the speed-only rung sits in the same table and the same set of videos
without being reinterpreted. ``speed_adjusting.evaluate`` builds on this too,
so there is exactly one episode loop for this method.

    python -m Centralized_Local_Planner.local_path_replanning.speed_adjusting.adapter --seeds 5
    python -m Centralized_Local_Planner.local_path_replanning.speed_adjusting.adapter --render --seed 0

What the planner does: order the active AMRs by TTC, try the speed factors
1, 2/3, 1/3, 0 fastest-first, commit the first whose 5 s rail rollout stays out
of every worker's Step-B hard lobe and out of the higher-priority AMRs'
reservations, reserve it, and STOP when none is admissible. Every active AMR is
shielded every frame; there is no cluster hand-over and no busy area.

Note the caveat this carries into the comparison table: because it shields
every AMR every frame rather than only the members of a conflict cluster, its
"AMR-frames replanned" and "shield override %" do not mean the same thing as
the other three methods' and are reported as n/a rather than misreported.
"""
...
from .planner import (
    ACTION_GO, ACTION_STOP, SpeedAdjustingConfig, SpeedAdjustingReplanner,
)

_REPO = Path(__file__).resolve().parents[3]
_TRAIL = 30
_AMR_AMR_DIST = 0.60      # m, centre-to-centre physical overlap

# The tuned configuration this planner was evaluated with (see
# outputs/5_local_path_replanning/results.json).
TUNED = SpeedAdjustingConfig(shield_steps=25, reserve_clearance=1.0)


class _TimedReplanner(SpeedAdjustingReplanner):
...
def run(seed: int = 0, frames: int = 560, workers: int = 2, amrs: int = 6,
        cfg: SpeedAdjustingConfig | None = None, snapshots: bool = False,
        shielded: bool = True):
    """Run one episode. Returns (metrics, snapshots|None).

    ``shielded=False`` removes the planner entirely: the AMRs drive at the
    QR-planner speed with no local replanning at all. That is the no-shield
    baseline ``evaluate`` compares against.
    """
    replanner = _TimedReplanner(cfg or TUNED) if shielded else None
    ...
    for f in range(frames):
        out = pipe.step(f)
        plan_ms.append(replanner.last_ms if replanner is not None else 0.0)

        actions = dict(replanner.last_actions) if replanner is not None else {}
        speeds = dict(replanner.last_speeds) if replanner is not None else {}
        ...
        for i in range(len(live)):
            for j in range(i + 1, len(live)):
                if float(np.linalg.norm(live[i][1] - live[j][1])) < _AMR_AMR_DIST:
                    amr_amr_pairs.add(tuple(sorted((live[i][0], live[j][0]))))
    ...
    metrics = dict(
        ...
        amr_amr_collisions=float(len(amr_amr_pairs)),
...
def main(argv=None):
    ...
    if args.render:
        ...
        out = Path(args.out) if args.out else (
            _REPO / "outputs" / "5_local_path_replanning" / "demos"
            / "demo_speed_adjusting.mp4")
        out.parent.mkdir(parents=True, exist_ok=True)
        build(snaps, None, "speed_adjusting", out, fps=args.fps)
        print(f"saved -> {out}")
    ...
    print(f"\nseeds={list(range(args.seeds))} frames={args.frames} "
          f"amrs={args.amrs} workers={args.workers}   (speed adjusting)")
    print("=" * 44)
