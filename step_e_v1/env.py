"""Cluster-level RL environment for training the V1 attention policy.
...
from .action_decoder import decode_action_np
from .config import V1Config
from .observation_builder import build_cluster_observation_np
...
class ExternalProposalPlanner(ShieldedReplanner):
    ...
    def __init__(self, config):
        ...
        self._fallback = V0SequentialReplanner(config)
...
class ClusterEnv:
    ...
    def __init__(self, cfg: V1Config, env_cfg: EnvConfig | None = None,
    ...
    def _harvest(self, use_disk: bool) -> list:
        ...
        for seed, n_w, n_a in self.scenes():
            ...
            rt = StepERuntime(V0SequentialReplanner(self.cfg), self.cfg,
    ...
    def reset(self, index: int):
        ...
        self.rt = StepERuntime(self.planner, self.cfg, num_frames=e.frames,
    ...
    def step(self, raw_action: np.ndarray):
        ...
        # bounded action, so decode here -- exactly as V1AttentionReplanner does
...
def _snapshot(rt: StepERuntime) -> dict:
...
def _restore(rt: StepERuntime, state: dict) -> None:
