"""Space-time reservation safety shield -- shared by V0 and V1.
...
def select_safe_trajectory(agent, candidates, worker_predictions, reservation,
    ...
    ``prefer_proposal=False`` (V0): accept the lowest-cost SAFE candidate.
    ``prefer_proposal=True``  (V1): execute the proposal whenever it is safe,
    ...
    # Lazy candidate set (see V1Config.lazy_backups): check the proposal on its
